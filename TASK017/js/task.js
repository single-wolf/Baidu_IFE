...
function renderChart() {
    var container=document.getElementById("aqi-chart-wrap"),
          city=["北京","上海","广州","深圳","成都","西安","福州","厦门","沈阳"],
          time=["day","week","month"],
          width=["7px","30px","100px"],
          color=["#666","#4F4F2F","#2F4F2F","green","#5C4033","blue","yellow","#9F5F9F","red","#aaa","black"];
    var str="";
    var data=chartData[city[pageState.nowSelectCity]][time[pageState.nowGraTime]];
    for(var i=0;i<data.length;i++){
      str+="<div title='日期:"+getDay(i,pageState.nowGraTime)+"\n污染指数:"+data[i]
          +"' style='height:"+data[i]+"px;background-color:"+color[Math.floor(data[i]/50)]
          +";width:"+width[pageState.nowGraTime]+"'></div>";
    }
    container.innerHTML = str;

    function getDay(i, flag){
        switch(flag){
          case 0 : return date[city[pageState.nowSelectCity]][i]; break;
          case 1 : return "2016年第" + (i+1) + "周"; break;
          case 2 : return "2016年第" + (i+1) + "月"; break;
        }
    }
}
...
function initCitySelector() {
  ...
  for(var city in aqiSourceData){
      select.options.add(new Option(city));
  }
